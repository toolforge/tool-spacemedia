...
public interface Video2CommonsTaskRepository extends CrudRepository<Video2CommonsTask, String> {
    ...
    @Query("select m from #{#entityName} m where m.status = 'FAIL' and not exists(select x from #{#entityName} x where x.status = 'DONE' and (x.url = m.url or x.metadataId = m.metadataId))")
