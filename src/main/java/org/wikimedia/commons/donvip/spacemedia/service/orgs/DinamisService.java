...
public class DinamisService extends AbstractOrgService<DinamisMedia> {
    ...
    DinamisMedia mapMedia(DinamisFeature feature, String collection) throws IOException {
        ...
        media.setPolygon(new Polygon(
                feature.geometry().coordinates().stream().map(x -> new Point(x.get(0).get(0), x.get(0).get(1)))
                        .toList()));
        addMetadata(media, getDownloadLink(props.fid()), null);
        return media;
    }

    String getDownloadLink(int fid) throws IOException {
        try (CloseableHttpClient httpclient = HttpClientBuilder.create().build()) {
            HttpClientContext context = getHttpClientContext(cookieStore);
            // STEP 1 - post fid
            HttpUriRequestBase request = newHttpPost(RESOURCES_URL,
                    Map.of("val_fid", "{\"values\":[" + Integer.toString(fid) + "]}"));
            ...
            request.setEntity(MultipartEntityBuilder.create().setMode(HttpMultipartMode.STRICT)
                    .addPart("profil", sb("Particulier"))
                    .addPart("theme", sb("Autres"))
                    .addPart("pays", sb("fra"))
